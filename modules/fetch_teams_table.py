    ...
    for player in player_info.values():
        ...
        if team_code not in teams_data:
            teams_data[team_code] = {
                ...
                # raw totals too (if you need them)
                "starts": 0, "minutes": 0, "clean_sheets": 0, "assists": 0,
                "expected_assists": 0, "goals_scored": 0, "expected_goals": 0,
                "yellow_cards": 0, "red_cards": 0, "bps": 0, "bonus": 0,
                "dreamteam_count": 0, "starts_benched": 0, "minutes_benched": 0,
                "own_goals": 0, "goals_conceded": 0, "penalties_saved": 0, "penalties_missed": 0,
            }

        # --- Fantasy team-style aggregates (fallbacks applied) ---
        ...
        teams_data[team_code]["clean_sheets_team"] += player.get(
            "clean_sheets_team", 0)
        teams_data[team_code]["assists_team"] += player.get("assists_team", 0)
        ...
        # bps: prefer team key, else raw
        bps_team_val = player.get("bps_team")
        if bps_team_val is None:
