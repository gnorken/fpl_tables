    ...
    with ThreadPoolExecutor(max_workers=8) as ex:
        ...
        for fut in as_completed(future_to_gw):
            ...
            for el in elements:
                ...
                # Handle captain points for multipliers 2 (captain) or 3 (triple captain)
                base_points = stats.get("total_points", 0)
                if mult == 2:
                    # 1x base points for regular captain
                    summary["captain_points_team"] += base_points
                elif mult == 3:
                    summary["captain_points_team"] += base_points * \
                        2  # 2x base points for triple captain
                ...
                summary["total_points_team"] += base_points * \
                    mult  # Full points including captain multiplier
